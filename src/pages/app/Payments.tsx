  ...
  return (
    <div className="px-8 py-10">
      ...
      <p className="mt-1 text-sm text-charcoal/60">
        {isBrand
          ? "Fund one balance. Approve milestones as they land — release is instant, on your terms."
          : "Get paid the moment your work is approved, not on someone else's payment run."}
      </p>
      ...
      <div className="mt-6 flex gap-3">
        {isBrand ? (
          <button
            ...
            className="rounded-md bg-charcoal px-4 py-2.5 text-sm font-medium text-white hover:bg-charcoal/90"
          >
            Fund balance
          </button>
        ) : (
          <button
            ...
            className="rounded-md bg-signal px-4 py-2.5 text-sm font-medium text-white hover:bg-signal/90 disabled:opacity-40"
          >
            Withdraw instantly
          </button>
        )}
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 rounded-md bg-charcoal px-4 py-3 text-sm text-white shadow-lg">
          {toast}
        </div>
      )}
    </div>
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: "gold" | "aurora" | "signal" }) {
  ...
  return (
    <div className="rounded-lg border border-charcoal/10 bg-white p-5">
      ...
      <div className={clsx("mt-2 font-display text-2xl", color)}>{value}</div>
    </div>
  );
}
